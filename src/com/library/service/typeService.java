...
//	查询全部类型名称
	public ArrayList findTypeName() {
		//	获取全部部门名称
		ArrayList<String> type_list = new ArrayList<String>();
		try{
			ResultSet rs = db.getRs("select type from type");
			while(rs.next()){
...
//	查询数据
	public String findTypezxt(String chkName, String chkYear) {
		String desc = "";
...
//				一天
				...
				ResultSet rs = db.getRs("select t.type," +
						...
						"from record r,person p,type t where r.cartNum = p.cartNum and p.type = t.id group by t.type");
				while(rs.next()){
...
//				一周
				...
				System.out.println("select t.type," +
